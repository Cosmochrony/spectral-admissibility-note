...
\hypersetup{
  ...
  pdfsubject={Map of the spectral admissibility sub-programme: constituent papers, phases,
    and the status of their principal results},
  pdfkeywords={spectral admissibility, Born--Infeld constraint, Weil representation,
    Heisenberg group, pair observable, capacity exponent, lepton mass hierarchy, SU(3),
    colour triplet, non-injective projection, presentation note}
}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    central question: which sectors of a supplied finite Heisenberg carrier and its associated
    Weil action survive the Born--Infeld bounded-flux constraint?
    The investigation organises the entire O-series (papers O1 and O3--O33, five precursor papers, and
    ...
    This note maps the constituent papers, identifies the internal phases, records the
    status of their principal results as proved, structural, numerical, conditional, or open, and states the
    ...
    identification of the spin-$\tfrac{1}{2}$ sector; and a full-breadth numerical campaign beyond
    $q = 211$.
    \textit{Interpretive status.}
    This map distinguishes internal mathematical results from supplied carriers and open bridges;
    it does not derive a physical gauge or matter sector.
  \end{abstract}
  ...
  \section{Motivation and Central Question}
    ...
    \medskip
    \noindent\textit{Central question.} The spectral construction studies the proposed
    non-injective projection $\Pi$ on supplied representation blocks $V_c$ of
    $\Heis_3(\mathbb{Z}/q\mathbb{Z})$, together with the associated Weil action; the blocks are
    indexed by characters $c \in (\mathbb{Z}/q\mathbb{Z})^\times$.
    ...
    $\betastar \in (0.09, 0.13)$ that O3 fixes by matching the charged-lepton sector;
    ...
    This note is a structured entry point, not a substitute for the constituent papers.
  ...
  \section{Position in the Cosmochrony Programme}
    ...
    The selection of $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ from those axioms is \emph{not} a
    theorem: HeisenbergStructure~\cite{Beau2026HeisenbergStructure} disproves that implication by
    an explicit six-element countermodel, and the Weil action is a further supplied object, so this
    sub-programme's mathematics is conducted on a supplied carrier.
    Branch~II is the spectral admissibility sub-programme (this note).
    Branch~III develops the quantum, spacetime-geometric, gauge, and fermionic-matter tracks using
    Branch~I axioms and Branch~II spectral data, subject to the status of each connecting bridge.
    ...
    It takes the supplied Heisenberg carrier and associated Weil action as inputs and organises two
    items used by Branch~III:
    \begin{enumerate}
      \item the admissibility thread $Q_8 \subset 2I \subset \SU(2)$, within which the
      spin-$\tfrac{1}{2}$ sector is selected by O26 minimality on a carrier supplied by O23 rather
      than identified here, that identification remaining an open deliverable;
      \item the measured capacity exponent $\dimpair$ on the supplied construction; its assignment
      to Hilbert--Schmidt norm growth in the minimal non-abelian sector $\su(2)$ remains conditional
      on the carrier and identification premises.
...
Throughout this note, \emph{admissibility constraint} denotes the bounded-flux condition.
...
$\chi$ itself.
  ...
  \section{Logical Chain of the Sub-Programme}
    ...
    sub-problem with its own evidentiary status.
    ...
    O15~\cite{Beau2026a19} states the Heisenberg transfer no-go, any resolution requiring a new
    growth carrier or an explicit cross-substrate hypothesis; and the Span-Growth
    Note~\cite{Beau2026sgn} proves that the expander-derived conversion law does not transfer to
    the Heisenberg substrate.
    ...
    \begin{enumerate}
      ...
      The factor-of-two relation $\dimpair = 2\delta_c$ follows from a further hypothesis of
      O16~\cite{Beau2026a20}, that the conjugate-pair ratio is independent of the shell index.
      On the exact block observable that ratio is identically one, so the pair product reduces to
      the single block squared and no separate doubling arises there.

      \item The \emph{observable hierarchy}: the block capacity $\sigma_c(n)$ (normalised BFS
      ...
      admissibility; $2I$ dominates its competitors at valence $d = 24$, its maximality among all
      finite $\SU(2)$ subgroups remaining a conjecture; the LPS family provides the Ramanujan
      approximation to $\SU(2)$ in the large-$q$ limit.
    \end{enumerate}
  ...
  \section{Constituent Papers}
    ...
    \subsection{Precursor papers: SU(2) admissibility sub-programme}
      ...
      Five papers precede the O-series proper and explore spectral admissibility on Cayley graphs
      of finite $\SU(2)$ subgroups.

      \textbf{SpectralAdmissibility}~\cite{Beau2026a1} carries out the spectral admissibility
      programme on the chain $Q_8 \subset 2I \subset \SU(2)$.
      ...
      At the valences $d \in \{6, 12, 24\}$ it proves binary cover dominance by a finite algebraic
      computation on classical character tables, the dominant group being the binary polyhedral
      group of the valence: $Q_8$ at $d = 6$, $2O$ at $d = 12$, $2I$ at $d = 24$.
      ...
      \textbf{ThreeGenerations}~\cite{Beau2026a4} shows that a scalar power-law threshold gives a
      featureless stratigraphic profile, while a threshold acting representation by representation
      on ADE-type binary Cayley graphs yields exactly three non-zero spectral classes for four
      pairs $(G, S)$ in $\{2T, 2O, 2I\}$, read there as the generation count (structural).

      \textbf{SpectralRelaxation}~\cite{Beau2026a5} derives
      $\Lambda_{\mathrm{proj}} \asymp h(G)^2$ from the isoperimetric capacity of the cascade
      graph, which gives $\Lambda_{\mathrm{proj}} \asymp \lambda_2$ for expander families
      satisfying spectral-isoperimetric saturation.
      At fixed LPS prime the spectral gap converges, so $\Lambda_{\mathrm{proj}}$ becomes
      asymptotically static and stabilisation is governed by spectral counting instead.
    ...
    \subsection{O-series core: LPS phase (O1--O8)}
      ...
      \textbf{O1}~\cite{Beau2026a6} shows that the asymptotic saturation of
      $\Lambda_{\mathrm{proj}}$ at fixed valence is an artefact of holding the LPS parameter
      fixed: letting the valence grow contracts the Kesten--McKay support towards its
      midpoint, restores a non-trivial dynamical regime for $\Lambda_{\mathrm{proj}}$, and
      corrects the fixed-valence ordering inversion.

      \textbf{O3}~\cite{Beau2026a7} obtains the hierarchical amplification mechanism from a
      growing relational valence, and fixes the compatibility window $\betastar \in (0.09, 0.13)$
      by matching the charged-lepton sector, anchored on $m_e/m_\tau$ (structural).

      \textbf{O4}~\cite{Beau2026a8} examines whether a structural upper bound on the cascade
      exponent follows from the BI flux constraint together with the Cheeger isoperimetric
      inequality, and finds that it does not follow from that argument.
      In O4's own convention $p(n) \sim n^\beta$ the candidate bound is $\beta \leq 2$, and the
      rescaling that would turn it into $\beta \leq 1$ would equally move O3's window to
      $(0.045, 0.065)$.
      ...
      \textbf{O7}~\cite{Beau2026a11} reformulates the target as a decay condition on the
      discrete projective capacity: find a fingerprint scale and depth for which
      $\bar\Sigma_n \sim p(n)^{-\delta}$ with $\delta \in [7.4, 10.6]$ in a measurable
      pre-saturation window (structural).

      \textbf{O8}~\cite{Beau2026a12} constructs the three-step path fingerprint at ambient
      dimension $O(q^3)$ and tests that condition on LPS graphs, where exponential shell growth
      compresses the pre-saturation window into $O(\log q)$ BFS steps and prevents stable slope
      estimation: the geometric obstruction that closes the LPS phase.
    ...
    \subsection{O-series core: Heisenberg transition (O9--O15)}
      ...
      Set $G_q = \Heis_3(\mathbb{Z}/q\mathbb{Z})$.
      The Cayley graph $\Cay(G_q, S_q)$ has polynomial ball growth
      ...
      representation-level ambiguity left by O11, and fits
      $\hat\delta_{\mathrm{exact}} \in \{4.37, 4.28, 4.80\}$ at $q \in \{29, 53, 61\}$
      ($R^2 > 0.98$).
      It also derives the observable in closed form and shows it exactly independent of the
      translational and central coordinates by an algebraic identity, so those fitted slopes are a
      finite-$q$ crossover statistic and the intrinsic unfolded exponent is
      $\delta_{\mathrm{exact}} = 3$.

      \textbf{O13}~\cite{Beau2026a17} extends the computation to $q \in \{101, 151, 211\}$ and
      rules out the finite-size hypothesis in its strong form: the exact-block exponent does not
      drift toward the phenomenological range at larger primes, the measurement becoming strictly
      more reliable there.
      It requalifies the $\delta$--$\betastar$ tension rather than resolving it: which quantity is
      the physical one --- the finite-$q$ crossover slope, the unfolded exponent
      $\delta_{\mathrm{exact}} = 3$, or the saturation-scale scaling function --- is left open.
    ...
    \subsection{Pair observable and transfer boundary (O16--O24)}
      \label{sec:pair}
      ...
      O16 proposes the fibre-level \emph{pair capacity}
      $\spair(n) = \sigma_c(n) \cdot \sigma_{q-c}(n)$ on conjugate pairs $\{c, q-c\}$, under the
      fibre and realisation hypotheses stated below.

      \textbf{O16}~\cite{Beau2026a20} examines whether conjugate blocks are the fibres of $\Pi$
      and types the two premises that identification would need, neither derived here (structural).
      ...
      \textbf{O17}~\cite{Beau2026a21} proves, within a scalar Weil-representation toy model, that
      the raw Gram--Schmidt redundancy count is exactly independent of the central character and of
      both initial-condition parameters --- identical for any two blocks, not only for conjugate
      ones.
      The proof is combinatorial and uses no conjugation identity; the reading of the conjugate
      pair as a fibre remains a hypothesis.
      ...
      \textbf{O20}~\cite{Beau2026a24} proposes a projective persistence criterion for selecting
      the physical sub-spectrum, the selecting mechanism being unidentified there (structural).
      ...
      \textbf{O22}~\cite{Beau2026a26} proves a shell-alignment theorem.
      Born--Infeld projection locking forces $\nobs$ onto a BFS shell of $\Cay(G_q, S_q)$.
      The alignment is not an independent geometric hypothesis.
    ...
    \subsection{Numerical campaign and sector-status papers (O25--O33), with the companion notes}
      \label{sec:numerical}
      ...
      \textbf{O26}~\cite{Beau2026a30} constructs a quadratic completion of admissible spectral
      pairs on a supplied binary-icosahedral carrier.
      It formulates a hierarchy of identifications: growth equivalence at Level~I, quotient
      identification at Level~II, and the conditional canonical identification of the
      spin-$\tfrac{1}{2}$ sector at Level~III, for which Test~4 states a falsifiability criterion.
      The doublet dimension it selects is a minimality choice, not a measurement.
      ...
      \textbf{O28}~\cite{Beau2026a32} tests the BFS window calibration out of sample, rejects the
      fitted linear extrapolation, and reports the adjusted $\delta_{\mathrm{corr}}(q)$ within
      ...
      derivation.
      Results for the test primes $q \in \{61, 151, 211, 307\}$ ($q \equiv 1 \pmod 3$) and the
      controls $q \in \{29, 101\}$ ($q \equiv 2 \pmod 3$).
      Four triplets are tested at each test prime.
      Against the noise floor $\mathtt{ctrl\_ref} = 1.657 \times 10^{-3}$ and the criterion
      $R \leq 3$, the normalised ratios are $R = 2.1$ at $q = 61$, $0.4$ at $q = 151$, $1.0$ at
      $q = 211$ and $0.7$ at $q = 307$: the test passes at all four primes.
      At $q = 61$ only one of the four triplets yields a valid $\delta_{\mathrm{tri}}$ estimate,
      the other three returning NaN from short sigma profiles; O32 records this as a known pipeline
      artefact at that prime and states that it does not affect the variance ratio, which is
      computed from all four triplets.

      \textbf{O33}~\cite{Beau2026a37} gives a finite spectral architecture for mass-square
      splitting and mixing: exact doublets and protected sectors from Weil admissibility, proved.
      It makes no identification with a Standard-Model representation, and derives neither Dirac
      masses, nor an absolute mass scale, nor generations, nor hypercharge, nor a matter-sector
      Higgs mechanism.
      ...
      \begin{table}[ht]
        \centering
        \caption{Summary of the corpus by phase.
        Section~\ref{sec:numerical} presents the last four rows together.
        ...
        \begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.7cm}
          ...
          \midrule
          SU(2) precursors & SpAdm, SpCap, SpGram, 3Gen & $Q_8 \subset 2I$; binary cover dominance,
            $d = 6, 12, 24$ & P/S \\
          LPS phase & SpRel, O1--O8 & Geometric obstruction; no cascade-exponent bound & P/O \\
          Heisenberg transition & O9--O15 & Crossover slope $\approx 4.5$, unfolded exponent
            $\delta_{\mathrm{exact}} = 3$; conditional bound & P/C \\
          Pair + transfer boundary & O16--O24 & $\cchi \to \dimpair$ fibre-conditionality closed & C \\
          ...
          Spectral architecture & O33 & Exact doublets and protected sectors, no
            Standard-Model identification & P \\
          Companion notes & Span-Growth, Critical Coverage & Transfer failure; exact depth law & P \\
          \bottomrule
        \end{tabular}
      \end{table}

  \section{Inputs and Supplied Structures}
    \label{sec:inputs}

    The spectral admissibility sub-programme takes the following inputs.
    The status of each item states whether Branch~I derives it or the spectral construction supplies it.

    \begin{enumerate}
      \item \textbf{Supplied Heisenberg carrier and Weil action.}
      An irreducible carrier for $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ and its associated Weil action
      are supplied.
      ...
      HeisenbergStructure~\cite{Beau2026HeisenbergStructure} disproves the selection of the finite
      Heisenberg group and its irreducible carrier from the axioms A1--A4, by an explicit
      six-element countermodel; the Weil action is a further representation of the symplectic
      automorphism group, which Stone--von Neumann does not by itself supply.
      Both are therefore supplied realisations, and the mathematics of this sub-programme is
      conducted on them.

      \item \textbf{BI saturation functional.}
      The sub-programme takes the Born--Infeld saturation form and its constant $\cchi$ as supplied
      inputs; Foundation presents that form as an explicit candidate rather than deriving its
      uniqueness from A1--A4.
      ...
      No independent dimensional parameter can be introduced at the admissibility level without
      breaking the axioms or BFS-consistent scaling~\cite{Beau2026j}, so the only dimensionful
      parameter available to the sub-programme is $\cchi$.
      The stronger statement, that this determines the capacity law functionally, is left open
      there.
    \end{enumerate}

    \noindent No result from Branch~III is an input to the sub-programme.
    The admissibility thread $Q_8 \subset 2I \subset \SU(2)$ and the measured value $\dimpair$
    enter downstream work with the statuses stated above; $\betastar$ is not an output, the
    conversion to it having no carrier on this substrate.

  \section{Interface with Downstream Branches}
    \label{sec:outputs}

    Table~\ref{tab:outputs} lists the principal items handed to downstream consumers, including
    supplied or conditional inputs rather than only derived outputs.

    \begin{table}[ht]
      \centering
      \caption{Principal interfaces of the spectral admissibility sub-programme and their consumers.
      ...
      \begin{tabular}{@{}>{\raggedright\arraybackslash}p{5.5cm}
        ...
        \midrule
        Measured $\dimpair$ on the conditional pair construction; the reciprocal
          $\betastar \approx 0.126$ is a coincidence check
          & SMchargemass, lepton hierarchy (coincidence only) & N/C \\
        Admissibility thread $Q_8 \subset 2I \subset \SU(2)$; binary cover dominance verified at
          $d \in \{6, 12, 24\}$ & Q1--Q3 (quantum sector) & P/C \\
        Spin-$\tfrac{1}{2}$ sector $V_\rho \cong \mathbb{C}^2$ (supplied, not derived) & Q7--Q11 (geometry) & C \\
        $\Sigma_c(n_3) = 3$ selection rule and conditional adjoint dimension
          & Q7 (spatial identification) & C \\
        O32 $[\mathrm{H\text{-}color}]$ measurements; no SU(3) identification
          & Q6a, Gauge note & N/O \\
        $\delta_{\mathrm{corr}}(q) \in [7.7, 9.0]$ for $q \in \{61,\ldots,211\}$, a declared
          adjustment; O25 assigns the window agreement to the raw exponent
          & Open: $q \to \infty$ limit & N \\
        \bottomrule
      \end{tabular}
    \end{table}
  ...
  \section{Status of Results}
    ...
    \subsection{Proved results}
      ...
      \begin{enumerate}
        \item \textit{Admissibility envelope} (SpectralAdmissibility~\cite{Beau2026a1}):
        in the linearised regime it proves that
        $A_n^{\max}=\cchi/\sqrt{\lambda_n}$, and the exact one-mode dynamics on $Q_8$ preserves
        the same saturation envelope.

        \item \textit{Binary cover dominance at the verified valences}
        (SpectralCapacity~\cite{Beau2026a2}): for $d \in \{6, 12, 24\}$ the binary polyhedral
        group of that valence dominates its competitors --- $Q_8$ at $d = 6$, $2O$ at $d = 12$ and
        $2I$ at $d = 24$ --- by a finite algebraic computation.
        The maximality of $2I$ among all finite $\SU(2)$ subgroups remains a conjecture there.
        ...
        \item \textit{Spectral-isoperimetric relation}
        (SpectralRelaxation~\cite{Beau2026a5}):
        $\Lambda_{\mathrm{proj}} \asymp h(G)^2$, and
        $\Lambda_{\mathrm{proj}} \asymp \lambda_2$ for expander families satisfying
        spectral-isoperimetric saturation.
        ...
        \item \textit{Block-independence in the scalar toy model} (O17~\cite{Beau2026a21}): the
        raw Gram--Schmidt redundancy count is independent of the central character and both initial
        conditions within that model; no transfer to the O12/O13 pipeline is asserted.
        ...
        irreducible carrier of complex dimension two,
        $\dim_\mathbb{R} \Im\mathbb{H} = 3$ (Theorem~3.1);
        ...
        \item \textit{Exact doublets and protected sectors} (O33~\cite{Beau2026a37}): the finite
        spectral architecture for mass-square splitting and mixing, with no identification of any
        Standard-Model representation.

        \item \textit{Failure of the capacity-to-rate transfer} (Span-Growth
        Note~\cite{Beau2026sgn}): the expander-derived conversion law does not transfer to the
        Heisenberg measurement substrate.

        \item \textit{Exact saturation-depth law} (Critical Coverage
        Note~\cite{Beau2026ccnote}): $n_1(q) \to 22$ in probability with
        $x_1(q) \asymp q^{-2}$, and deterministic bounds for every prime $q \geq 311$.
    ...
    \subsection{Structural results}
      ...
      \begin{enumerate}
        \item \textit{Representation-wise three-level stratigraphy}
        (ThreeGenerations~\cite{Beau2026a4}): four tested ADE-type binary Cayley-graph pairs yield
        exactly three non-zero spectral classes, read there structurally as a generation count.

        \item \textit{Phenomenological window} (O3~\cite{Beau2026a7}): $\betastar \in (0.09, 0.13)$,
        matching the charged-lepton sector and anchored on $m_e/m_\tau$.

        \item \textit{Capacity exponent target} (O7~\cite{Beau2026a11}): the decay condition
        $\bar\Sigma_n \sim p(n)^{-\delta}$ with $\delta \in [7.4, 10.6]$ in a measurable
        pre-saturation window; the three-step construction that tests it is O8's.

        \item \textit{Conditional pair-product construction} (O16~\cite{Beau2026a20}): under a
        constant conjugate-pair ratio, the pair product doubles the log-log slope.
        On the exact block observable that ratio is identically one, so the product reduces to the
        single block squared and no doubling phenomenon arises there; O16 asserts no value of
        $\dimpair$.

        \item \textit{Projective-persistence proposal} (O20~\cite{Beau2026a24}): under two
        structural hypotheses, a saturation-threshold criterion selects a conditional exponent
        window; the selecting mechanism and the threshold remain open there.
    ...
    \subsection{Conditional results}
      ...
      The following hold only under premises that the sub-programme does not derive.

      \begin{enumerate}
        \item \textit{The conjugate-pair construction} (O16~\cite{Beau2026a20}): conditional on
        identifying conjugate blocks as fibres of $\Pi$ and realising the fibre involution as
        $c \leftrightarrow q-c$; neither premise is derived there.
    ...
    \subsection{Numerical evidence}
      ...
      \begin{enumerate}
        ...
        $q \in \{61, 101, 151, 211\}$ after the finite-size adjustment.
        O25 assigns the window agreement to the raw exponent, which descends into
        $[7.4, 10.6]$ unaided and reaches $7.61$ at $q = 601$, the adjustment overcorrecting to
        $6.53$ there.
        ...
        over four triplets at each test prime, the normalised ratio $R$ satisfies the criterion
        $R \leq 3$ at $q \in \{61, 151, 211, 307\}$, with values $2.1$, $0.4$, $1.0$ and $0.7$
        against the noise floor $1.657 \times 10^{-3}$.
      \end{enumerate}
    ...
    \subsection{Open hypotheses}
      ...
      \begin{enumerate}
        ...
        \item \textit{Breadth of the campaign}: the pair-level campaign is complete at full
        breadth at the five baseline primes through $q = 211$, and O25 extends it to
        $q \in \{307, 401, 601\}$ at reduced sampling, so the large-$q$ dispersion there is
        inflated by the reduced pair count.
      \end{enumerate}
  ...
  \section{Open Deliverables}
    ...
    Of its four levels, two are established and two remain open.
    ...
    The pair-level campaign runs at the five baseline primes through $q = 211$ at full breadth.
    O25 extends it to $q \in \{307, 401, 601\}$ at reduced sampling ($M = 16$ at $307$, $M = 8$
    and twelve pairs at $401$ and $601$), so the large-$q$ dispersion there is inflated by the
    reduced pair count.
    At $q = 307$ the O32 test gives window variance $1.17 \times 10^{-3}$ and normalised ratio
    $R = 0.7$, inside its criterion~\cite{Beau2026a36}.
    What remains is breadth rather than reach: a full-breadth run beyond $q = 211$ would
    supply uninflated data points, both for the approach of $n_1(q)/q$ to the limit the Critical
    Coverage Note establishes and for the $R_{\mathrm{var}}(q) \propto q^{-1}$ scaling.

    \medskip
    \noindent The list above is limited to programme-level deliverables.
    Constituent-level open problems remain governed by their source papers and are not classified
    here as harmless to downstream claims.
  ...
  \section{Conclusion}
    ...
    Cosmochrony corpus.
    Starting from a supplied Heisenberg carrier, its associated Weil action, and the Born--Infeld
    bounded-flux constraint, it measures the capacity exponent $\dimpair$, records O26's minimality
    selection of a spin-$\tfrac{1}{2}$ sector on a supplied carrier, and organises the admissibility
    thread $Q_8 \subset 2I \subset \SU(2)$ with its proved and conjectural parts separated.
    ...
    The numerical campaign is complete at full breadth for $q \in \{29, 61, 101, 151, 211\}$ and
    extended at reduced sampling to $q \in \{307, 401, 601\}$.
    ...
    The supplied inputs, conditional identifications, and derived outputs mapped here are used
    throughout Branch~III: by the quantum sector
    ...
    The map shows that the programme's durable content is an archipelago of internal mathematical
    and numerical results connected by supplied or open bridges, rather than a closed derivation of
    gauge structure or matter phenomenology.
    Its role is to make those evidentiary boundaries explicit before downstream interpretations are
    assessed.
