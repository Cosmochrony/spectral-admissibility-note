...
\begin{document}
  ...
  \begin{abstract}
    ...
    along a single derivation chain:
    \[
      \text{bounded flux}
      \;\Longrightarrow\;
    ...
    \]
    Its transfer segment is closed by O24; its group-theoretic extensions are not.
...
$\chi$ itself.
  ...
  \approx 4.3$--$4.8$ for $q \in \{29, 53, 61\}$ ($R^2 > 0.98$).
    ...
    \subsection{Numerical campaign and closure papers (O25--O32)}
      ...
      $\delta_{\mathrm{corr}}(q) \in [7.7, 9.0]$ fall within the target window $[7.4, 10.6]$ for
      $q \in \{61, 101, 151, 211\}$.
  ...
  \section{Status of Results}
    ...
    \subsection{Structural results}
      ...
      The SU(3) emergence framework formerly listed here rested on the metaplectic identity that
      O31~\cite{Beau2026a35} withdraws, and is not asserted.
  ...
  \section{Open Deliverables}
    ...
    \textbf{Open deliverable 3: Numerical campaign at $q = 401$.}
  ...
  \section{Conclusion}
    ...
    unconditionally, selects the spin-$\tfrac{1}{2}$ admissible sector by minimality, and
