...
$\chi$ itself.
  ...
  \approx 4.3$--$4.8$ for $q \in \{29, 53, 61\}$ ($R^2 > 0.98$).
    ...
    \subsection{Numerical campaign and closure papers (O25--O32)}
      ...
      SU(3) is identified as the symmetry group of the co-admissible colour triplet, conditional
      on hypothesis~$[\mathrm{H\text{-}color}]$ for the standard generating set.
  ...
  \section{Status of Results}
    ...
    \subsection{Open hypotheses}
      ...
      \begin{enumerate}
        \item \textit{$[\mathrm{H\text{-}color}]_{\mathrm{pointwise}}$}: O32~\cite{Beau2026a36}
        establishes three of the four levels of the hypothesis analytically
        (rank equality in O31, average equality to $O(q^{-1})$, effective exponent equality in
        the $q \to \infty$ limit).
        The remaining open level is exact pointwise profile equality at finite $q$ and $M$.
        The spectral route to this level is closed (O31): the correct mechanism must act directly
        on the rank structure of the BFS walk, not on Markov spectra.
  ...
  \section{Open Deliverables}
    ...
    \textbf{Open deliverable 1: Pointwise proof of $[\mathrm{H\text{-}color}]$.}
    O32~\cite{Beau2026a36} establishes a three-tier resolution of this hypothesis.
    (i)~$[\mathrm{H\text{-}color}]_{\mathrm{rank}}$: sector rank equality
    $r_c(n) = r_{\omega c}(n) = r_{\omega^2 c}(n)$ is proved unconditionally in
    O31~\cite{Beau2026a35} via intra-coset Vandermonde symmetry.
    (ii)~$[\mathrm{H\text{-}color}]_{\mathrm{avg}}$: the block-averaged capacity satisfies
    $\mathbb{E}[\sigma_c(n)] = \mathbb{E}[\sigma_{\omega c}(n)] + O(q^{-1})$, proved in
    O32 (Proposition~1) via the modulation frequency argument.
    (iii)~$[\mathrm{H\text{-}color}]_{\mathrm{eff}}$: the effective exponents satisfy
    $\delta_c = \delta_{\omega c} = \delta_{\omega^2 c}$ in the $q \to \infty$ limit, proved in O32.
    What remains open is $[\mathrm{H\text{-}color}]_{\mathrm{pointwise}}$: exact profile equality
    at finite $q$ and $M$, which is consistent with all numerical results but not claimed
    analytically.
    The SU(3) identification on the standard graph is therefore analytically established at the
    level of effective exponents; the remaining open question is the finite-$q$ residual variance
    $R_{\mathrm{var}}(q) \propto q^{-1}$, to be confirmed at $q = 401$.
  ...
  \section{Conclusion}
    ...
    and supported on the standard graph at the rank, averaged, effective-exponent, and
    numerical levels (O31--O32,~\cite{Beau2026a35,Beau2026a36}).
    The remaining open component of $[\mathrm{H\text{-}color}]$ is the exact pointwise
    finite-$q$ profile equality on the standard graph.
