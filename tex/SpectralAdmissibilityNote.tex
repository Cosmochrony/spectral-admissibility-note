  ...
  \approx 4.3$--$4.8$ for $q \in \{29, 53, 61\}$ ($R^2 > 0.98$).
    ...
    \subsection{Numerical campaign and closure papers (O25--O32)}
      ...
      \[
        ...
        \Rightarrow H_{\mathrm{eff}} \cong \Sym^2(V_\rho)\;\text{(spin-$1$)}\;[\text{O29}]
        \Rightarrow r_{\mathrm{eff}} = 3\;\text{(carrier)},\; d_\rho = 2\;\text{(square-root)}\;[\text{O29}].
      \]
      ...
      \textbf{O29}~\cite{Beau2026a33} identifies the carrier $H_{\mathrm{eff}} \cong \Sym^2(V_\rho)$
      (spin-$1$): $r_{\mathrm{eff}} = 3$ confirmed numerically for $q \in \{61, 101, 151\}$, with $d_\rho = 2$
      the spin-$\tfrac{1}{2}$ square-root doublet recovered via the Veronese structure, not the rank measured
      by Test~4.
  ...
  \section{Outputs to Downstream Branches}
    ...
    \begin{table}[ht]
      ...
      \begin{tabular}{@{}p{5.5cm}p{4.5cm}l@{}}
        ...
        SU(3) co-admissible triplet (unconditional, O31~v1.5) & Q6a, Gauge note & P \\
