...
$\chi$ itself.
  ...
  \approx 4.3$--$4.8$ for $q \in \{29, 53, 61\}$ ($R^2 > 0.98$).
    ...
    \subsection{Numerical campaign and closure papers (O25--O32)}
      ...
      through the admissibility quotient, under $\ker \Pi_{\mathrm{adm}} \subseteq \ker \Phi$, and
      exhibits the canonical representation action $\rho \colon \su(2) \to \End(V_\rho)$; the
      corresponding intertwiner space is one-dimensional, so an intertwiner is unique only up to a
      scalar.
      Identifying the quotient with $\su(2)$ is the carrier supplied conditionally by O23.
      The vector lift of O26 Hypothesis~4.4 is not furnished and remains open, since
      $\operatorname{Hom}_{\su(2)}(\mathbf{3},\mathbf{2}) = 0$.
      The identification chain becomes, with arrows of unequal type --- its first step being the
      carrier supplied conditionally by O23, not a consequence of O27's three constraints:
      \[
        \text{emergence} \Rightarrow \su(2)\;[\text{O27, conditional}]
        \Rightarrow H_{\mathrm{eff}} \cong \Sym^2(V_\rho)\;\text{(spin-$1$)}\;[\text{O29}]
        \Rightarrow r_{\mathrm{eff}} = 3\;\text{(carrier)}\;[\text{O29}].
      \]
      ...
      (spin-$1$): $r_{\mathrm{eff}} = 3$ confirmed numerically for $q \in \{61, 101, 151\}$.
      the doublet dimension $d_\rho = 2$ is the minimality selection of O26, not a measurement of those data, and no Veronese square root is recovered there.
      This is the carrier rank, not the rank measured
      by Test~4.

      \textbf{O30}~\cite{Beau2026a34} gives a conditional structural account of an eigenvalue ratio
      $\lambda_1/\lambda_2 = 2$, conditional on (H0)--(H3) there and, for the spectral ratio, on the two weighted
      decorrelation conditions; it concerns a $3\times3$ model on $\Sym^2(V_\rho)$ and not the
      $9\times9$ covariance measured in O28.
      The factor of two is the squared symmetric-tensor normalisation of the direction that the
      Carnot grading identifies as central.
      The numerical $9\times9$ ratio reported in O28 is an empirical result, independent of that
      account.
  ...
  \section{Status of Results}
    ...
    \subsection{Proved results}
      ...
      \begin{enumerate}
        ...
        morphism factors uniquely through the admissibility quotient; identifying that quotient
        with $\su(2)$ is conditional on O23's carrier.

        \item \textit{Eigenvalue ratio} (O30~\cite{Beau2026a34}): in a conditional $3\times3$
        model on $\Sym^2(V_\rho)$, the ratio $\lambda_1/\lambda_2 = 2$ follows from the equatorial
        constraint under (H0)--(H3) and two weighted decorrelation conditions.  It is not an
        account of O28's $9\times9$ measurement.
    ...
    \subsection{Numerical evidence}
      ...
      \begin{enumerate}
        ...
        confirmed for $q \in \{61, 101, 151\}$; the doublet dimension $d_\rho = 2$ is the minimality selection of O26, not a measurement of those data.
  ...
  \section{Conclusion}
    ...
    $\Heis_3(\mathbb{Z}/q\mathbb{Z})$, it estimates the cascade exponent $\betastar \approx 0.126$,
    selects the spin-$\tfrac{1}{2}$ admissible sector by minimality, and establishes the
    admissibility thread $Q_8 \subset 2I \subset \SU(2)$ as a structural output.
    The spin-$\tfrac{1}{2}$ sector is not identified unconditionally: it is not reachable
    equivariantly from the adjoint, and O26 Hypothesis~4.4 remains open.

    The SU(2) sector is carried by a conditional analytical chain, not closed.
    It is closed numerically for $q \in \{29, 61, 101, 151, 211, 307\}$.
    The $\SU(2)$ sector is not analytically closed: its carrier is supplied conditionally by O23
    and O26 Hypothesis~4.4 remains open.
